...
# Импортируйте в файл birthday/views.py класс ListView и создайте
# CBV BirthdayListView — этот класс заменит view-функцию
# birthday_list():
...
# Создаём миксин. От него унаследуют свойства
# BirthdayCreateView и BirthdayUpdateView
class BirthdayMixin:
    model = Birthday
...
# Создадим миксин BirthdayFormMixin, который будем
# «подмешивать» только к CBV создания и редактирования объекта.
...
class BirthdayCreateView(BirthdayMixin, CreateView):
    ...
    # fields = '__all__'
    # А можно подключить форму вместо атрибута fields нужно указать атрибут
    #  form_class; значением этого атрибута будет BirthdayForm.
    # form_class = BirthdayForm

    # Явным образом указываем шаблон:
    # template_name = 'birthday/birthday.html'
    # Можно не указывать, но шаблон должен иметь адрес по умолчанию
...
# birthday_list на обращение к методу as_view()
# класса BirthdayListView. Этот метод есть у всех встроенных
...
class BirthdayDeleteView(BirthdayMixin, DeleteView):
    pass
    # model = Birthday
    #   Создали отдельную страницу birthday_confirm_delete.html
    #   строка template_name не нужна
    #   template_name = 'birthday/birthday.html'
    # success_url = reverse_lazy('birthday:list')
    # Создадим для страницы удаления объекта отдельный шаблон.
    #  Дадим ему имя, которое рекомендуется в документации, —
    # имя-модели_confirm_delete.html: birthday_confirm_delete.html.
    #  Как и в других CBV, в DeleteView можно не указывать
    #  имя шаблона, если его имя соответствует ожидаемому.
